 <?php
/**
 * WooCommerce PDF Invoices Settings Page/Tab
 *
 * @author      Bas Elbers
 * @category    Admin
...
if ( ! class_exists( 'BEWPI_Settings_Page' ) ) :

/**
 * WC_Settings_Page.
 */
abstract class BEWPI_Settings_Page {
	...
	public function get_settings() {
		return apply_filters( 'bewpi_get_settings_' . $this->id, array() );
	}
	...
	public function get_sections() {
		return apply_filters( 'bewpi_get_sections_' . $this->id, array() );
	}
	...
	public function output_sections() {
		...
		foreach ( $sections as $id => $label ) {
			echo '<li><a href="' . admin_url( 'admin.php?page=bewpi-settings&tab=' . $this->id . '&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section === $id ? 'current' : '' ) . '">' . $label . '</a> ' . ( end( $array_keys ) === $id ? '' : '|' ) . ' </li>';
		}
